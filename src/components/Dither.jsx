...
function DitheredWaves({
  ...
  useFrame(({ clock }) => {
    ...
    // seed a time offset so the very first frame is already a rich pattern
    // (the raw t=0 frame is nearly flat, which flashes blank on load)
    if (!disableAnimation) {
      u.time.value = clock.getElapsedTime() + 6.0;
    } else {
      // reduced motion: freeze on a good-looking static frame
      u.time.value = 6.0;
    }
